function C = spt_link_cost(P, Q, R, useEr, lambda, sup)
%SPT_LINK_COST  Frame-to-frame link cost matrix (np x nq). Euclidean distance, Inf beyond R.
%   When useEr, multiply by (1 + lambda·off) where off = spt_seg_off_fraction (straight-line off-ER
%   fraction) — the 'penalty' linking mode.
dx = P(:,1) - Q(:,1).'; dy = P(:,2) - Q(:,2).';
C  = hypot(dx, dy);
C(C > R) = Inf;
if useEr && ~isempty(sup)
