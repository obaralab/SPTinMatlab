function L = spt_tiff_labels(tiffPath)
%SPT_TIFF_LABELS  What the acquisition itself says each page of a stack is.
%
...
% single nominal `finterval`. The timepoint index is what exists, and it is exact.
%
% WHY A SINGLE-COLOUR PIPELINE WANTS THIS. Two things it otherwise has to assume:
%
%   * THAT A STACK'S PAGES ARE ITS FRAMES. A stack straight off the microscope may be several
%     channels interleaved — `_ch24_spt.tif` here is 5,000 pages but 2,500 timepoints of two colours
%     — and tracking it as 5,000 frames links a molecule to the OTHER colour's exposure on every
%     step, at half the true interval. Nothing about the page count says so; the labels do.
%   * THAT AN ORGANELLE STACK HAS A PAGE PER TRACKED FRAME. `page = frame + 1` is right only when the
%     two stacks are on the same footing. Where the movie is interleaved and the segmentation is one
%     page per timepoint, that rule is wrong by the number of channels, and past the end of the
%     shorter stack it silently clamps to the last page for the rest of the movie. See spt_page_map.
%
% A de-interleaved stack carries labels too: `c:1/4 t:1/5981` on a stack extracted from a four-channel
% acquisition, with its timepoints renumbered to its own pages. That is the case where page and
% timepoint agree — and it is worth being able to tell it apart from the case where they do not.
%
% OUTPUT L
