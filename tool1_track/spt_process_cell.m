...
% prm : struct with linkUm, gapUm, maxGap, erAware, lambda, pxUm, dtS; optional maxFrames,
...
% pxUm, nFrames, nTracks, haveMito, erAware. Feed to spt_write_outputs.
px = prm.pxUm;
...
% ---- ER masks (per-spot ER distance + ER-aware linking supports) + mito masks ----
haveEr   = ~isempty(cel.erSeg) && isfile(cel.erSeg);
modeReq = ''; if isfield(prm,'linkMode') && ~isempty(prm.linkMode), modeReq = prm.linkMode; end
if isempty(modeReq), if prm.erAware, modeReq = 'penalty'; else, modeReq = 'euclid'; end, end
...
mode = modeReq;
if ~prm.erAware || ~haveEr, mode = 'euclid'; end
erAware = ~strcmp(mode,'euclid');
...
[tracks, tinfo] = spt_track(dets, supports, prm.linkUm, prm.gapUm, prm.maxGap, px, erAware, prm.lambda, mode);
...
R = struct('key',cel.key,'base',base,'sptPath',cel.spt,'erPath',erPath,'mitoPath',mitoPath, ...
    ...
    'haveMito',haveMito,'haveEr',haveEr,'erAware',erAware, ...
