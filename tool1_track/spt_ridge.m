function [r, keep, wMaj] = spt_ridge(dog, px, py, ridgeMax, scale, sizeMax)
%SPT_RIDGE  Principal-curvature ratio at candidate peaks — rejects RIDGE (filament) responses.
%
%   r                = spt_ridge(dog, px, py)
%   [r, keep, wMaj]  = spt_ridge(dog, px, py, ridgeMax, scale, sizeMax)
%
...
%
% INPUT
%   dog      : the DoG detection image (from spt_dog)
%   px, py   : integer column/row indices of the candidate peaks (must be >=2 px from the border,
%              which spt_detect already guarantees — the Hessian is a 3x3 central difference)
%   ridgeMax : the largest curvature RATIO R to accept. [] or <=0 disables the shape test.
%   scale    : spt_dog's `scale` output — this spot's size relative to the reference. Required for
...
if ~isempty(ridgeMax) && ridgeMax > 0
    ...
    if ridgeMax < 1
        warning('spt_ridge:ratioBelowOne', ...
