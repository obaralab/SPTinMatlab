function [xy, shp, rej] = spt_detect(frame, diamUm, pxUm, thrAbs, opts)
...
%                       SHAPE half of the bleedthrough filter (see spt_ridge).
...
%                       aligned bleedthrough the curvature ratio is blind to (see spt_skel_align).
...
% caller asked for it. Compute it here rather than re-entering spt_detect to get it, which would
% redo the DoG for nothing.
doAlign = ~isempty(alignDeg) && alignDeg > 0 && isfield(opts,'skel') && ~isempty(opts.skel);

[dog, hp, dscale] = spt_dog(frame, diamUm, pxUm);
if isempty(thrAbs)
...
if (~isempty(ridgeMax) && ridgeMax > 0) || (~isempty(sizeMax) && sizeMax > 0)
    [~, kr] = spt_ridge(dog, px, py, ridgeMax, dscale, sizeMax);
...
% Alignment rejection runs LAST, because it needs the shape the loop above just measured. Its
% rejects join `rej` so the preview draws them with the rest — from the user's side there is one
% gate, however many criteria it is made of.
if doAlign
    dropA = spt_skel_align(opts.skel, xy, shp, opts);
    if any(dropA)
        if nargout > 2, rej = [rej; xy(dropA,:)]; end
        xy = xy(~dropA,:); shp = shp(~dropA,:);
    end
end
end
