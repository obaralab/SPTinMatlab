function q = spt_pool_quality(sptPath, diamUm, pxUm, nSample, opts)
...
% Sample only the pages belonging to this channel. On an interleaved stack the untouched version
% pooled organelle frames into the quality distribution the percentile threshold is read off.
...
for k = idx
    ...
    [dog, ~, dscale] = spt_dog(f, diamUm, pxUm);
    ...
    cand = (dog >= mx) & (dog > thr);
    frameIdx = (k - offset - 1)/stride + 1;          % page -> frame number the gate is defined on
    ...
    if gateThis && ((~isempty(ridgeMax) && ridgeMax > 0) || (~isempty(sizeMax) && sizeMax > 0))
        ...
        [~, kr] = spt_ridge(dog, cxq, cyq, ridgeMax, dscale, sizeMax);
