function [a, b] = spt_channel_stem(varargin)
%SPT_CHANNEL_STEM  The one rule for putting a tracked channel into a file name, and taking it out.
%
%   stem        = spt_channel_stem(base, key)   % compose:  'cellA' + 'ch2' -> 'cellA__ch2'
%   [base, key] = spt_channel_stem(stem)        % split:    'cellA__ch2'    -> 'cellA', 'ch2'
%
...
if nargin == 2
    ...
    if isempty(key) || strcmp(key, spt_channel_stem_default())
        a = base;
    else
        a = [base '__' key];
    end
...
function k = spt_channel_stem_default()
