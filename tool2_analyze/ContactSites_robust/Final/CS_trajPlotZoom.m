function CS_trajPlotZoom(CSstruct, index, TrajArray)
   ...
   % close all
    ...
    figure;
    plot(cs_config().FrameInt_s*CSstruct(index).CSmatrix(:,TrajArray,1),sqrt(CSstruct(index).CSmatrix(:,TrajArray,2).^2+CSstruct(index).CSmatrix(:,TrajArray,3).^2),'LineWidth',1.5); % per-run dt (was 0.011)
    ylim([-0.5 3.5]);
    %xlim([0 2500]);
    hold on
    yline(0,'--');
