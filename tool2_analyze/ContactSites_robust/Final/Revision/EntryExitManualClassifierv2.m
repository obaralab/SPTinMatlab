...
for i=1:size(index,2) % By contact site
    ...
    for j=1:size(bindingEvents,2) %By bound trajectory
        ...
        figure(2)
        hold on
        traj1=plot(cs_config().FrameInt_s*CSstruct(index(i)).CSmatrix(:,bindingEvents(j),1),sqrt(CSstruct(index(i)).CSmatrix(:,bindingEvents(j),2).^2+CSstruct(index(i)).CSmatrix(:,bindingEvents(j),3).^2),'LineWidth',3,'Color','r'); % per-run dt (was 0.011)
        ...
        for k=1:NumBind %By binding interaction
            ...
            figure(1)
            dt=cs_config().FrameInt_s; % per-run dt (was 0.011); EntryPts/ExitPts are seconds -> /dt gives frame index
            SegInd=find((T>CSstruct(index(i)).DwellTimes(bindingEvents(j)).EntryPts(k,1)/dt).*(T<CSstruct(index(i)).DwellTimes(bindingEvents(j)).ExitPts(k,1)/dt));
