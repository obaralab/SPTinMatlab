function [CSdwellTimes] = DwellTimeManual(CSstruct,index,TrackStruct)
...
for i=1:size(index,2)
    ...
    for j=1:size(CSstruct(index(i)).tracks,2)
        ...
        figure(2)
        hold on
        traj1=plot(cs_config().FrameInt_s*CSstruct(index(i)).CSmatrix(:,j,1),sqrt(CSstruct(index(i)).CSmatrix(:,j,2).^2+CSstruct(index(i)).CSmatrix(:,j,3).^2),'LineWidth',3,'Color','r'); % per-run dt (was 0.011)
