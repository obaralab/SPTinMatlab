...
% One reusable, OFF-SCREEN figure for all the per-CS reference images: saveas
% renders it to disk without flashing a window on screen or leaving one open.
...
%     Col14(i)=size(CS(i).neighborDeff,1);
    ...
    % Save density plots
    set(groot,'CurrentFigure',fig1);   % target the off-screen figure (no raise/flash)
    hold off
...
if isgraphics(fig1), close(fig1); end   % done rendering per-CS images
