function M = spt_time_match(tA, tB, opts)
%SPT_TIME_MATCH  For each time in A, the nearest time in B — or nothing, when B was not looking.
%
%   M = spt_time_match(tA, tB)
%   M = spt_time_match(tA, tB, opts)
%
...
assert(ismember(rule,{'nearest','previous','next'}), 'spt_time_match:rule', ...
